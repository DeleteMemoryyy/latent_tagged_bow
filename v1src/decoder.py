...
def decoding_infer(start_id,
    ...
    def _dec_loop_attn_fn(i, prev_id, dec_state, dec_outputs, dec_out_index):
        ...
        if(sampling_method == "greedy"):
            ...
            dec_prob_topk, dec_id_topk = tf.nn.top_k(
                dec_prob, topk_size)  # [B, K]
            dec_prob_topk /= tf.expand_dims(
                tf.reduce_sum(dec_prob_topk, 1), [1])

            sample = tf.multinomial(
                tf.log(dec_prob_topk), 1)  # [B, K] -> [B, 1]
            sample = tf.one_hot(tf.reshape(sample, [batch_size]), topk_size)
            dec_index = tf.reduce_sum(
                tf.cast(sample, tf.int32) * dec_id_topk, axis=1)
...
def decoding_train(dec_inputs,
    ...
    pointer_ent = tf.reduce_sum(-dec_pointers *
                                tf.log(dec_pointers + 1e-10), 2)
